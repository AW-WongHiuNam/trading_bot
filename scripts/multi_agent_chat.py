...
def fetch_and_store_news(
    ...
    *,
    ...
    """Call Alpha Vantage NEWS_SENTIMENT, store to SQLite, optionally return feed preview & immediate search hits."""
...
def handle_tool_request(text):
    ...
    for line in raw.splitlines():
        ...
        if name in {"fetch_news_full", "fetch_and_read"}:
            return (
                name,
                fetch_and_store_news(arg, return_feed=True, preview_n=None, do_search=True, return_full_feed=True),
                stripped,
            )
